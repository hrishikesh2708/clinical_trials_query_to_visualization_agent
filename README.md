...
Open [http://localhost:8501](http://localhost:8501). The sidebar lists the same **10 curated examples** (2 per horizon) documented under [Live examples](#live-examples-2-per-horizon). Optional `BACKEND_URL` in `.env` defaults to `http://127.0.0.1:8000`.
...
Ten live query examples below (**two per horizon**). Full JSON files are in [`examples/live/`](examples/live/). Five mocked fixture copies for offline tests remain in [`examples/`](examples/).

Each block is a collapsible `<details>` section (GitHub). The first example per horizon includes a captured JSON response; the second shows the curl request only.
...
To regenerate the [live examples](#live-examples-2-per-horizon) section, edit [`examples/live/sample_queries/manifest.json`](examples/live/sample_queries/manifest.json) and run:

```bash
uv run python scripts/render_sample_queries_section.py
```

## Live examples (2 per horizon)

Ten curated natural-language queries (two per horizon).
...
<details>
<summary><strong>Pembrolizumab trials per year since 2015 — time_series, 13 data rows, 1000 studies</strong></summary>
...
<details>
<summary><strong>Recruiting lung cancer trials by country — bar_chart, 68 data rows, 1000 studies</strong></summary>
...
<details>
<summary><strong>Countries running pembrolizumab trials</strong></summary>
...
<details>
<summary><strong>Diabetes sponsor and drug network — network_graph, 56 nodes, 74 edges, 15 studies</strong></summary>
...
Saved to [`examples/live/network_diabetes_sponsor_drug.json`](examples/live/network_diabetes_sponsor_drug.json) (39,964 bytes).
